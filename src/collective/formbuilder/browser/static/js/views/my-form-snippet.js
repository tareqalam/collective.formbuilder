...
function(Backbone, SnippetView, TempSnippetView, PubSub) {
  ...
  return SnippetView.extend({
    ...
    mouseDownHandler : function (mouseDownEvent) {
      ...
      $('.popover #save').on('click', this.saveHandler(self));
      $('.popover #cancel').on('click', this.cancelHandler(self));
      ...
      if (this.model.get('title') !== 'Form Name') {
        $('body').on('mousemove', function (mouseMoveEvent) {
          if (
            ...
            PubSub.trigger('mySnippetDrag', mouseDownEvent, self.model);
            self.mouseUpHandler();
          };
        });
      }
